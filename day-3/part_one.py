...
def find_intersection(x11, y11, x12, y12, x21, y21, x22, y22):
    ...
    x = 0
    y = 0
    (x11, y11, x12, y12) = normalize(x11, y11, x12, y12)
    (x21, y21, x22, y22) = normalize(x21, y21, x22, y22)
    if y11 == y12: # line 1 is horizontal
        if y21 != y22: # line 2 is vertical
            if x11 < x21 < x12 and y22 < y12 < y21:
                (intersects, x, y) = (True, x22, y11)
    else:  # line 1 is vertical
        if x21 != x22: # line 2 is horizontal
            if x21 < x11 < x22 and y12 < y22 < y11:
                (intersects, x, y) = (True, x11, y22)
    return (intersects, x, y)


def get_coords(x, y, segment):
    ((x1, y1), (x2, y2)) = ((x, y), (0, 0))
    if segment[0] == 'U':
        y2 = y1 + int(segment[1:])
        x2 = x1
...
def get_coords_list(wire):
    ...
    for segment1 in wire:
        ((x1, y1), (x2, y2)) = get_coords(x1, y1, segment1)
...
def find_intersections(wire1, wire2):
    ...
    for coord1 in coords1:
        ...
        for coord2 in coords2:
            ((x21, y21), (x22, y22)) = coord2
            (intersects, x, y) = find_intersection(x11, y11, x12, y12, x21, y21, x22, y22)
            if intersects:
                result.append((x, y))
    return result


def find_min_distance(coords):
    min_distance = abs(coords[0][0]) + abs(coords[0][1])
    for coord in coords:
        current = abs(coord[0]) + abs(coord[1])
