...
def find_intersection(steps1, x11, y11, x12, y12, steps2, x21, y21, x22, y22):
    intersects = False
    x = 0
    y = 0
    steps = steps1 + steps2
    (nx11, ny11, nx12, ny12) = normalize(x11, y11, x12, y12)
    (nx21, ny21, nx22, ny22) = normalize(x21, y21, x22, y22)
    if ny11 == ny12: # line 1 is horizontal
        if ny21 != ny22: # line 2 is vertical
            if nx11 < nx21 < nx12 and ny22 < ny12 < ny21:
                intersects = True
                steps += abs(x21 - x11) + abs(y12 - y21)
    else:  # line 1 is vertical
        if nx21 != nx22: # line 2 is horizontal
            if nx21 < nx11 < nx22 and ny12 < ny22 < ny11:
                intersects = True
                steps += abs(x11 - x21) + abs(y22 - y11)
    return (intersects, steps)


def get_coords(steps, x, y, segment):
    ((x1, y1), (x2, y2)) = ((x, y), (0, 0))
    ...
    if segment[0] == 'L':
        ...
        x2 = x1 + distance
        y2 = y1
    coords = (steps, (x1, y1), (x2, y2))
    return coords


def get_coords_list(wire):
    result = []
    (steps, (x1, y1), (x2, y2)) = (0, (0, 0), (0, 0))
    steps = 0
    newsteps = 0
    for segment1 in wire:
        (newsteps, (x1, y1), (x2, y2)) = get_coords(steps, x1, y1, segment1)
        result.append((steps, (x1, y1), (x2, y2)))
        steps = newsteps
        (x1, y1) = (x2, y2)
    return result
...
def find_intersections(wire1, wire2):
    result = []
    coords1 = get_coords_list(wire1)
    coords2 = get_coords_list(wire2)
    for coord1 in coords1:
        (steps1, (x11, y11), (x12, y12)) = coord1
        for coord2 in coords2:
            (steps2, (x21, y21), (x22, y22)) = coord2
            (intersects, steps) = find_intersection(steps1, x11, y11, x12, y12, steps2, x21, y21, x22, y22)
            if intersects:
                result.append(steps)
    return result
...
def main():
    ...
    min_distance = functools.reduce(lambda x, y: x if x < y else y, intersections)
    print(min_distance)
